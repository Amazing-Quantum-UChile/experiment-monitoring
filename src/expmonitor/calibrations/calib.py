...
class Calibrator:
    ...
    def calibrate(self, show=True):
        ...
        self.popt, self.pcov = curve_fit(
            fit_fctn, self.data_x, self.data_y, p0=initial_guess, bounds=bounds
        )
        ...
        if show is True:
            ...
            ax.set_yscale("log")
            ax.semilogy(self.data_x, self.data_fitted, "b", label="Measurement + Fit")
            ax.semilogy(self.data_x, self.data_pfeiffer, "r", label="Pfeiffer")
            ax.set_xlabel("Voltage [V]")
            ax.set_ylabel("Pressure [mbar]")
            ax.legend()
            ax.set_title(
                r"Fit results: $P(V) = %.2f \times 10^{%.2f \times V - %.2f} + %.2f$"
